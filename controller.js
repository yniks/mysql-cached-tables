...
async function getTableController({
    table,
    database,
    tableNameSingular = table.endsWith('s') ? table.slice(0, -1) : table
}) {
    pool = await pool
    ...
    const {
        columns,
        key
    } = await pool.query(`describe ${tableid}`).then(result => ({
        columns: result.map(el => el.Field).reduce((prev, curr) => {
            prev.push(curr);
            return prev
        }, []),
    ...
    async function exists(arg) {
        return !!await find(arg).catch(_ => false)
    }
    /**
     * 
    ...
    async function getByKey(arg) {
        return find({
            [key]: arg[key]
        })
    }
    async function update(arg) {
        var serializedForUpdate = SerializeDataForUpdate({
            ...arg,
            [key]: undefined
        })
        var serializedForConds = SerializeConditions({
            [key]: arg[key]
        })
